...
const char* ssid = "wifi-seamless.id"; // Ganti dengan SSID WiFi Anda
const char* password = "3366998877"; // Ganti dengan password WiFi Anda
...
void loop() {
  ...
  String message = "{\"nim\":\"G.231.21.0181\",\"suhu\":" + String(t) + ",\"kelembaban\":" + String(h) + "}";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(nim) == "G.231.21.0181" || String(nim) == "all" || String(nim) == "ALL") { // Sesuaikan dengan nim Anda
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("G.231.21.0181", mqtt_user, mqtt_password)) {
